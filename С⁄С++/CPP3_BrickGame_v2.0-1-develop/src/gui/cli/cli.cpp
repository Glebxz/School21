...
void PrintField(WINDOW *game_field) {
  ...
  for (int i = 0; i < length_field; i++) {
    for (int k = 0, j = 0; j < width_field; j++, k++) {
...
void PrintTable(WINDOW *score_table) {
  ...
  mvwprintw(score_table, 6, 0, "Current score: %5d", game_info.score);
  mvwprintw(score_table, 7, 0, "High score: %5d", game_info.high_score);
  mvwprintw(score_table, 8, 0, "Level: %5d", game_info.level);
  mvwprintw(score_table, 9, 0, "Speed: %5d ms", game_info.speed);
  if (game_info.pause) {
